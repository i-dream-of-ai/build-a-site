...
const resendStream = async (
    ...
    if (response.status !== 200) {
      ...
      if (result.error) {
        ...
        const queue = encoder.encode(JSON.stringify({type: "error", content: result.error.message}));
        controller.enqueue(queue);
        controller.error(result);
      } else {
        
        const queue = encoder.encode(JSON.stringify({type: "error", content: `OpenAI API returned an error: ${result.statusText}`}));
        controller.enqueue(queue);
        controller.error(result);
      }
    }
...
export const getFunction = async (
    ...
    if (json.choices[0].delta.function_call.name) {
      ...
      const message = encoder.encode(JSON.stringify({type: "function_call", content: currentFunctionCall.name}));
      controller.enqueue(message);
    }
    if (json.choices[0].delta.function_call.arguments) {
      currentFunctionCall.arguments += json.choices[0].delta.function_call.arguments;
    }
};
...
export const runFunction = async (
    ...
    userId: string
      ...
      if(currentFunctionCall.name === 'generate_site'){
        ...
        // Resend stream
        await resendStream(model, systemPrompt, temperature, messages, encoder, new TextDecoder(), controller, streamParser);
