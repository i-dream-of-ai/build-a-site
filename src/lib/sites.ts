...
const updateSite = async ({ id, updates }: { id: string; updates: Site }): Promise<Site> => {
  try {
    const res = await fetch(`/api/sites/${id}`, {
      method: 'PATCH',
      headers: {
...
export const useSite = (id: string, enabled: boolean) => {
  return useQuery<Site, Error>(['sites', id], () => {
    return fetchSite(id);
  }, {
...
export const useSites = () => {
    return useQuery<Site[], Error>(['sites'], () => {
      return fetchSites();
    }, {
      onError: (error: unknown) => {
        if (error instanceof Error) {
...
export const useUpdateSite = () => {
  ...
  return useMutation<Site, Error, { id: string; updates: Site }, { previousSite: Site | undefined }>(updateSite, {
    ...
    onError: (error, variables, context) => {
      if (context?.previousSite) {
        queryClient.setQueryData(['sites', context.previousSite._id], context.previousSite);
      }
    },
