...
export function SiteList({sites:siteList}:SitesProps){
    ...
    return(
      ...
      {sites.map((site) => (
        <li key={site._id} className="flex items-center justify-between gap-x-6 py-5">
          <div className="min-w-0">
            <div className="flex items-start gap-x-3">
              <p className="text-sm font-semibold leading-6 text-neutral-300">{site.content.title}</p>
                ...
                <Menu.Items className="absolute right-0 z-10 mt-2 w-32 origin-top-right rounded-md bg-white py-2 shadow-lg ring-1 ring-gray-900/5 focus:outline-none">
                  ...
                  <Menu.Item>
                    {({ active }) => (
                      ...
                      >
                        Delete<span className="sr-only">, {site.bucketName}</span>
                      </button>
                    )}
                  </Menu.Item>
                </Menu.Items>
