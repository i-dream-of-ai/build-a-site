        ...
        if (event.type === 'event') {
          ...
          try {
            ...
            if(json.choices[0].finish_reason === 'length'){
              const queue = encoder.encode(JSON.stringify({type: "error", content: "Sorry, the chat has exceded the maximum length allowed."}));
      ...
      const onParseFunction = async (
        ...
        if (event.type === 'event') {
          ...
          try {
            ...
            if (json.choices[0].finish_reason != null) {
              try {
                await runFunction(encoder, controller, model, systemPrompt, temperature, messages, streamParser, userId);
              } catch (error) {
