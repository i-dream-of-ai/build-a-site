...
export async function deleteBucket(bucketName:string){
    try {
            ...
            for (const object of listObjectsResponse.Contents) {
                if (object.Key) {
                    try {
                        ...
                        console.error(`Could not delete object ${object.Key} from bucket ${bucketName}: ${deleteObjectErr}`);
                    }
                }
            }
        }
        ...
        console.error(`Could not list objects in bucket ${bucketName}: ${listObjectsErr}`);
    }
    ...
    try {
        ...
        response.error = `Could not delete bucket ${bucketName}: ${JSON.stringify(deleteBucketErr)}`;
    }
